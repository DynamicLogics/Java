...
public class ReceiveFoodOrder {
	...
	public OrderConfirmation receiveAndConfirmOrder(Order order) {
		try {
			System.out.println("[Food Order Received] receiveAndConfirmOrder "+order.getFoodName()+" "+order.getTableNumber());
			return new OrderConfirmation("order confirmed...");
		}catch (Exception e) {
			throw new ReplyFailureException("Failed to send reply with payload [" + order.getFoodName()+" "+order.getTableNumber()
			+ "]", e);
		}
	}
